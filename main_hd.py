    ...
    for data_setting in dataset_seq:
        ...
        for cm in cm_seq:
            ...
            for prod_setting in prod_seq:
                ...
                Model(dataset_name, product_name, cascade_model).model_hd()
                Model(dataset_name, product_name, cascade_model).model_hed()

                for wd in wd_seq:
                    wallet_distribution_type = 'm50e25' * (wd == 1) + 'm99e96' * (wd == 2)

                    ModelPW(dataset_name, product_name, cascade_model, wallet_distribution_type).model_hdpw()
                    ModelPW(dataset_name, product_name, cascade_model, wallet_distribution_type).model_hedpw()